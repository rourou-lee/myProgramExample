    ...
    function InputRuntime() {
        ...
        function enterInputMode() {
            ...
            if (node) {
                ...
                ////从外层获取上一次的节点名称
                //window.lastNodeText = receiverElement.textContent;
            }
        }
        ...
        function commitInputResult() {
            ...
            var node = minder.getSelectedNode();
            //设置节点字数限制  （ft）
            var text = commitInputText(textNodes);
            ...
            if (!window.lock && window.ManualEdit && window.socket) {
                ...
                //输入内容，点击enter 触发节点内容修改
                var selectNodeId = node.getData().id;
                window.socket.emit("nodeContentChange", { selectNodeId: selectNodeId, value: text });
            }
            // * ====================================

            commitInputNode(node, text);
