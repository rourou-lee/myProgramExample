/*!
 * ====================================================
 * Kity Minder Core - v1.4.50 - 2019-07-05
     ...
     */
         ...
         */
            execCommand: function(name) {
                ...
                // date: 20180426
                // content:同步执行命令。
                // 对于添加子节点、父节点、兄弟节点将生成ID放在具体执行方法之外，确保同步生成的节点ID一致；
                // 对于移动节点到父节点，需获取移动节点的ID和父节点的ID，同步移动；
                // * ====================================================
                var args = [];
                //主动生成节点id
                var createNode = [ "appendchildnode", "appendsiblingnode", "appendparentnode" ];
                if (createNode.indexOf(name) !== -1 && typeof ary[2] == "undefined") {
                    ary[2] = utils.guid();
                    args = ary;
                } else if (name === "movetoparent" && typeof ary[1] === "object" && typeof ary[2] === "object") {
                    var nodes = Array.prototype.slice.call(ary[1]);
                    var nodeIds = [];
                    for (var i = 0; i < nodes.length; i++) {
                        nodeIds.push(nodes[i].data.id);
                    }
                    var parentId = ary[2].data.id;
                    args[0] = name;
                    args[1] = nodeIds;
                    args[2] = parentId;
                } else if (name === "movetoparent" && typeof ary[2] !== "object") {
                    var moveNodes = this.getNodesById(ary[1]);
                    var parentNode = this.getNodeById(ary[2]);
                    ary[1] = moveNodes;
                    ary[2] = parentNode;
                } else if (name === "paste" && typeof ary[1] === "undefined") {
                    ary[1] = Date.now();
                ...
                // * ====================================================
                var cmdArgs = [].slice.call(ary, 1), cmd, stoped, result, eventParams;
                var me = this;
                ...
                if (!this._hasEnterExecCommand) {
                    ...
                    if (!stoped) {
                        ...
                        // add by shiyaru 
                        // date: 20180426
                        // content:排除放大缩小、移动视野、展开折叠的操作同步。
                        // * ====================================================
                        var excludeNameAry = [ "zoomin", "zoomout", "hand", "camera", "expandtolevel" ];
                        var isEdit = !window.lock && excludeNameAry.indexOf(name) === -1 && window.ManualEdit;
                        if (isEdit && window.socket) {
                            if (window.firstEdit) {
                                window.socket.emit("editLock");
                                window.firstEdit = false;
                            }
                            window.socket.emit("execCommand", {
                                params: args
                            });
                        }
                        // * ====================================================
                        ...
                        if (cmd.isContentChanged()) {
                            ...
                            // add by shiyaru 
                            // date: 20180426
                            // content:实时保存当前编辑的最新内容到缓存库。
                            // * ====================================================
                            // wy:
                            // if (isEdit && window.socket) {
                            //     window.socket.emit("saveContent", this.exportJson());
                            // }
                            // * ====================================================
...
_p[12] = {
    value: function(require, exports, module) {
         ...
         * @Date: 2015.9.20
         */
            importNode: function(node, json, num) {
                var data = json.data;
                node.data = {};
                for (var field in data) {
                    ...
                    // content:导入节点，确保id唯一
                    // * ====================================
                    if (field === "id" && typeof num !== "undefined") {
                        var newid = data[field].substring(12, 25) === "" ? data[field].substring(0, 12) + num : data[field].substring(12, 25) + num;
                        node.setData(field, newid);
                    } else {
                ...
                for (var i = 0; i < childrenTreeData.length; i++) {
                    ...
                    this.importNode(childNode, childrenTreeData[i], num);
                }
                return node;
            },
            /**
         ...
         */
            importData: function(protocolName, data, option) {
                ...
                return Promise.resolve(protocol.decode(data, this, option)).then(function(json) {
                    // if (json != undefined) {
                    //     json instanceof Promise ? json.then(function(json) {
                    //         minder.importJson(json);
                    //     }) : minder.importJson(json);
                    // }
                    // return json;
                    // wy: xmind tabs support
                    var arr = [];
                    if (json instanceof Array) {
                        arr = json;
                        for (var i = 1; i < arr.length; i++) {
                            arr[i] = compatibility(arr[i]);
                        }
                        json = json[0];
                    } else {
                        arr = json;
                    ...
                    if (arr instanceof Array) {
                        ...
                        for (var j = 0; j < arr.length; j++) {
                            ...
                            arr[j].tabName = arr[j].tabName || "画布" + (j + 1);
                            arr[j].tabId = tmp + timestamp;
                            arr[j].tabJson = JSON.stringify({
                                root: arr[j].root
                            });
                            delete arr[j].root;
                        }
     ...
     */
         ...
         */
            ...
            clone: function(num) {
                ...
                // add by shiyaru 
                // date: 20180510
                // content:克隆修改id，确保id唯一
                // * ====================================
                if (typeof num !== "undefined") {
                    var newid = cloned.data.id.substring(12, 25) === "" ? cloned.data.id.substring(0, 12) + num : cloned.data.id.substring(12, 25) + num;
                    cloned.data.id = newid;
                }
                // * ====================================
                this.children.forEach(function(child) {
                    cloned.appendChild(child.clone(num));
                });
                return cloned;
            },
        ...
        kity.extendClass(Minder, {
            ...
            createNode: function(textOrData, parent, index, id) {
                var node = new MinderNode(textOrData);
                // * ====================================
                // add by shiyaru 
                // date: 20180426
                // content:创建节点时，可指定节点id
                // * ====================================
        ...
        kity.extendClass(Minder, {
            ...
            select: function(nodes, isSingleSelect) {
                ...
                if (utils.isArray(nodes)) {
                    ...
                    nodeIds.push(nodes.data.id);
                }
                if (!window.lock && window.ManualEdit && window.socket) {
                    if (window.firstEdit) {
                        window.socket.emit("editLock");
                        window.firstEdit = false;
                    }
                    ...
                    if (_clipboardNodes.length) {
                        ...
                        for (var i = 0, ni; ni = _clipboardNodes[i]; i++) {
                            for (var j = 0, node; node = nodes[j]; j++) {
                                ...
                                // add by shiyaru 
                                // date:20180428
                                // content:复制的话给节点id重新赋值，确保所有的节点ID唯一
                                // * ====================================================
                                var copyNode = ni.clone(num);
                                appendChildNode(node, copyNode);
                            }
                        }
    ...
    value: function(require, exports, module) {
        ...
        var TreeDragger = kity.createClass("TreeDragger", {
            ...
            _enterDragMode: function() {
                ...
                this._dragMode = true;
                this._minder.setStatus("dragtree", true);
                return true;
            },
            // 从选中的节点计算拖放源
...
_p[47] = {
    value: function(require, exports, module) {
        ...
        Module.register("FlagModule", function() {
            var minder = this;
            var FLAG_DATA = "flag";
            var FLAG_COLORS = [ null, "#f25858", // 1 - red
            "#409eff", // 2 - blue
            "#f0a038", // 3 - orange
            "#0cb399", // 4 - blue green
            "#7187db", // 5 - purple
            ...
            "#7f8195", // 8 - gray
            "#eb6497" ];
            var FLAG_PATH = "M3.498887777328491,1.8067188262939453c-0.5035500526428223,0,-1.0553627014160156,0.4077749252319336,-1.0553627014160156,0.9124314785003662l0,15.045957803726196c0,0.5024623870849609,0.5394940376281738,0.9122428894042969,1.0421063899993896,0.9122428894042969c0.5022938251495361,0,1.0553627014160156,-0.4098186492919922,1.0553627014160156,-0.9124317169189453l0,-15.045637845993042c0,-0.5046374797821045,-0.5396251678466797,-0.9124313592910767,-1.0421063899993896,-0.9124313592910767ZM14.330239295959473,3.0589566230773926c-2.959425926208496,0,-2.959425926208496,-1.2130126953125,-5.918682098388672,-1.2130126953125c-1.708275318145752,0,-3.0273752212524414,0.7969498634338379,-3.0273752212524414,0.7969498634338379l-0.012337684631347656,9.080869674682617s1.331437587738037,-0.7958621978759766,3.039731025695801,-0.7958621978759766c2.959257125854492,0,2.959257125854492,1.2130126953125,5.918682098388672,1.2130126953125c1.8513736724853516,0,3.7110557556152344,-0.9720945358276367,3.7110557556152344,-0.9720945358276367l0,-9.07995057106018s-1.8596820831298828,0.9700686931610107,-3.7110557556152344,0.9700686931610107Z";
            // 旗子图标的图形
            var FlagIcon = kity.createClass("FlagIcon", {
                ...
                create: function() {
                    var frame = new kity.Path().setTranslate(-10, -10).setPathData(FLAG_PATH);
                    this.addShapes([ frame ]);
                    this.frame = frame;
                },
                setValue: function(value) {
                    this.frame.fill(FLAG_COLORS[value]);
                }
            });
            /**
         * @command Flag
         * @description 设置节点的旗子信息（添加一个旗子小图标）
         * @param {number} value 要设置的旗子
         *     取值为 0 移除旗子信息；
         *     取值为 1-n 表示各种颜色；
         * @state
         *    0: 当前有选中的节点
            ...
            renderers: {
                right: kity.createClass("hyperlinkrender", {
                    ...
                    update: function(link, node, box) {
                        ...
                        //svg的锚元素a标签没有title属性，链接再IE下不显示title，所以用svg的title标签代替
                        var title1 = document.createElementNS("http://www.w3.org/2000/svg", "title"), text = document.createTextNode(title);
                        title1.appendChild(text);
                        link.node.insertBefore(title1, link.node.childNodes[0]);
    ...
    value: function(require, exports, module) {
         ...
         */
            ...
            var ImageRenderer = kity.createClass("ImageRenderer", {
                base: Renderer,
                create: function(node) {
                    var icon = new kity.Image(node.getData("image"));
                    //点击节点图片，出现右侧图片上传20190410
                    icon.on("click", function() {
                        node.getMinder().fire("showUploadPicArea", {
                            node: node,
                            icon: icon,
                            pic: node.getData("image")
                        });
                    });
                    return icon;
                },
...
_p[54] = {
     ...
     */
        var AppendChildCommand = kity.createClass("AppendChildCommand", {
            base: Command,
            execute: function(km, text, id) {
                var parent = km.getSelectedNode();
                if (!parent) {
                    return null;
                }
                //导图插入下级节点改为插入到第一位
                var node = km.createNode(text, parent, 0, id);
                km.select(node, true);
     ...
     */
        var AppendSiblingCommand = kity.createClass("AppendSiblingCommand", {
            ...
            execute: function(km, text, id) {
                ...
                if (!parent) {
                    return km.execCommand("AppendChildNode", text, id);
                }
                var node = km.createNode(text, parent, sibling.getIndex() + 1, id);
     ...
     */
        ...
        var AppendParentCommand = kity.createClass("AppendParentCommand", {
            ...
            execute: function(km, text, id) {
                ...
                var newParent = km.createNode(text, parent, nodes[0].getIndex(), id);
                nodes.forEach(function(node) {
         ...
         */
            ...
            var NoteIconRenderer = kity.createClass("NoteIconRenderer", {
                ...
                create: function(node) {
                    ...
                    //点击节点备注图标，出现右侧备注编辑框20190410
                    icon.on("click", function() {
                        node.getMinder().fire("showRemarkEditArea", {
                            node: node,
                            icon: icon,
...
_p[57] = {
    value: function(require, exports, module) {
        ...
        var Module = _p.r(20);
        var Renderer = _p.r(27);
        Module.register("PortraitModule", function() {
            var minder = this;
            var PORTRAIT_DATA = "portrait";
            var PORTRAIT_COLORS = [ null, "#f25858", // 1 - red
            "#409eff", // 2 - blue
            "#f0a038", // 3 - orange
            "#0cb399", // 4 - blue green
            "#7187db", // 5 - purple
            ...
            "#7f8195", // 8 - gray
            "#eb6497" ];
            var PORTRAIT_PATH = "M5.3116841316223145,6.428452014923096c0,-2.699917793273926,2.1962199211120605,-4.896136999130249,4.896173000335693,-4.896136999130249c2.699918746948242,0,4.896137237548828,2.1962192058563232,4.896137237548828,4.896136999130249c0,2.7000784873962402,-2.196218490600586,4.896047115325928,-4.896137237548828,4.896047115325928C7.507904052734375,11.324499130249023,5.3116841316223145,9.128530502319336,5.3116841316223145,6.428452014923096L5.3116841316223145,6.428452014923096ZM1.2400001287460327,19.065807342529297c0.10541439056396484,-1.1839179992675781,0.6737757921218872,-5.378931045532227,5.0422316789627075,-7.6256208419799805c1.083460807800293,0.8504743576049805,2.4445786476135254,1.3625679016113281,3.9256434440612793,1.3625679016113281c1.4891357421875,0,2.8576269149780273,-0.5173006057739258,3.943988800048828,-1.3764734268188477c4.387354850769043,2.237238883972168,4.916985511779785,6.402849197387695,5.023850440979004,7.639402389526367C19.149656295776367,19.09088134765625,1.265897274017334,19.067758560180664,1.2400001287460327,19.065807342529297L1.2400001287460327,19.065807342529297ZM1.2400001287460327,19.065807342529297";
            // 肖像图标的图形
            var PortraitIcon = kity.createClass("PortraitIcon", {
                base: kity.Group,
                constructor: function(value) {
                    this.callBase();
                    this.setSize(20);
                    this.create();
                    this.setValue(value);
                    this.setId(utils.uuid("node_portrait"));
                    this.translate(.5, .5);
                },
                setSize: function(size) {
                    this.width = this.height = size;
                ...
                create: function() {
                    var frame = new kity.Path().setTranslate(-10, -10).setPathData(PORTRAIT_PATH);
                    this.addShapes([ frame ]);
                    this.frame = frame;
                },
                setValue: function(value) {
                    this.frame.fill(PORTRAIT_COLORS[value]);
                }
            });
            /**
         * @command Portrait
         * @description 设置节点的肖像信息（添加一个肖像小图标）
         * @param {number} value 要设置的肖像
         *     取值为 0 移除肖像信息；
         *     取值为 1-n 表示各种颜色；
         * @state
         *    0: 当前有选中的节点
         *   -1: 当前没有选中的节点
         */
            var PortraitCommand = kity.createClass("PortraitCommand", {
                base: Command,
                execute: function(km, value) {
                    var nodes = km.getSelectedNodes();
                    for (var i = 0; i < nodes.length; i++) {
                        nodes[i].setData(PORTRAIT_DATA, value || null).render();
                    }
                    km.layout();
                },
                queryValue: function(km) {
                    ...
                    var val;
                    for (var i = 0; i < nodes.length; i++) {
                        val = nodes[i].getData(PORTRAIT_DATA);
                        if (val) break;
                    }
                    return val || null;
                },
            ...
            return {
                commands: {
                    portrait: PortraitCommand
                },
                renderers: {
                    left: kity.createClass("PortraitRenderer", {
                        base: Renderer,
                        create: function(node) {
                            var icon = new PortraitIcon();
                            return icon;
                        },
                        shouldRender: function(node) {
                            return node.getData(PORTRAIT_DATA);
                        },
                        update: function(icon, node, box) {
                            var data = node.getData(PORTRAIT_DATA);
                            var spaceLeft = node.getStyle("space-left");
                            var x, y;
                            icon.setValue(data);
                            x = box.left - icon.width - spaceLeft;
...
_p[58] = {
    value: function(require, exports, module) {
        ...
        Module.register("PriorityModule", function() {
            ...
            // [MASK, BACK]
            var PRIORITY_COLORS = [ null, [ "#f25858", "#f25858" ], // 1 - red
            [ "#409eff", "#409eff" ], // 2 - blue
            [ "#f0a038", "#f0a038" ], // 3 - orange
            [ "#0cb399", "#0cb399" ], // 4 - blue green
            [ "#7187db", "#7187db" ], // 5 - purple
            [ "#38cf2d", "#38cf2d" ], // 6 - green
            [ "#c978ee", "#c978ee" ], // 7 - pink
            [ "#7f8195", "#7f8195" ], // 8 - gray
            [ "#eb6497", "#eb6497" ] ];
            // hue from 1 to 5
            // jscs:disable maximumLineLength
            var BACK_PATH = "M0 0L10 0A9,9,0,0,1,-10 1.102182119232618e-15A9,9,0,0,1,10 -2.204364238465236e-15L0 0z";
            var MASK_PATH = "M0 0L9 0A9,9,0,0,1,-9 1.102182119232618e-15A9,9,0,0,1,9 -2.204364238465236e-15L0 0z";
            ...
            var PriorityIcon = kity.createClass("PriorityIcon", {
                ...
                create: function() {
                    ...
                    back = new kity.Path().setPathData(BACK_PATH).setTranslate(10.5, 10.5);
                    mask = new kity.Path().setPathData(MASK_PATH).setOpacity(1).setTranslate(10.5, 10.5);
            ...
            return {
                ...
                renderers: {
                    left: kity.createClass("PriorityRenderer", {
                        base: Renderer,
                        create: function(node) {
                            var icon = new PriorityIcon();
                            //点击节点优先级图标，出现右侧进度图标选择框20190410
                            icon.on("click", function() {
                                node.getMinder().fire("showProgressSelectArea", {
                                    node: node,
                                    icon: icon,
                                    progress: node.getData(PRIORITY_DATA)
                                });
                            });
                            return icon;
                        },
            ...
            return {
                ...
                renderers: {
                    left: kity.createClass("ProgressRenderer", {
                        base: Renderer,
                        create: function(node) {
                            var icon = new ProgressIcon();
                            //点击节点进度图标，出现右侧进度图标选择框20190410
                            icon.on("click", function() {
                                node.getMinder().fire("showProgressSelectArea", {
                                    node: node,
                                    icon: icon,
                                    progress: node.getData(PROGRESS_DATA)
                                });
                            });
                            return icon;
                        },
     ...
     */
        ...
        var TextRenderer = kity.createClass("TextRenderer", {
            ...
            update: function(textGroup, node) {
                ...
                // wy:添加悬浮提示，控制显示长度
                var allNodeText = nodeText;
                nodeText = nodeText && nodeText.length > 20 ? nodeText.substring(0, 20) + "..." : nodeText;
                ...
                // wy:添加悬浮提示，控制显示长度
                var title1 = document.createElementNS("http://www.w3.org/2000/svg", "title"), text = document.createTextNode(allNodeText);
                title1.appendChild(text);
                textGroup.node.insertBefore(title1, textGroup.node.childNodes[0]);
                return function() {
        ...
        function encode(json, minder, option) {
            ...
            function drawSVG() {
                ...
                return loadImage(svgData).then(function($image) {
                    ...
                    // 出错后依然导出，只不过没有图片。
                    if (navigator.onLine) alert("脑图的节点中包含跨域图片，导出的 png 中节点图片不显示，你可以替换掉这些跨域的图片并重试。");
                    DomURL.revokeObjectURL(svgDataUrl);
                    document.body.appendChild(canvas);
                    var pngBase64 = generateDataUrl(canvas);
                    document.body.removeChild(canvas);
     ...
     */
        function Node2Text(node) {
            ...
            if (/^\s*$/.test(node.data.text)) {
                node.data.text = this && this.$t && this.$t("message.branchTheme") || "分支主题";
            }
...
_p[73] = {
    value: function(require, exports, module) {
        ...
        var markerMap = {
            ...
            "people-red": [ "portrait", 1 ],
            "people-blue": [ "portrait", 2 ],
            "people-orange": [ "portrait", 3 ],
            "people-dark-green": [ "portrait", 4 ],
            "people-purple": [ "portrait", 5 ],
            "people-green": [ "portrait", 6 ],
            "people-red": [ "portrait", 1 ],
            "flag-red": [ "flag", 1 ],
            "flag-blue": [ "flag", 2 ],
            "flag-orange": [ "flag", 3 ],
            "flag-dark-green": [ "flag", 4 ],
            "flag-purple": [ "flag", 5 ],
            "flag-green": [ "flag", 6 ]
        };
                ...
                function xml2km(xml) {
                    ...
                    if (Array.isArray(sheet)) {
                        for (var i = 0; i < sheet.length; i++) {
                            result = {};
                            processTopic(sheet[i].topic, result);
                            result.tabName = sheet[i].title || "";
                            arr.push(result);
                        }
                        return arr;
                    } else {
                        processTopic(sheet.topic, result);
                        return result;
                    }
                }
                function processJson(json, obj) {
                    // 处理文本
                    obj.data = {
                    ...
                    // 处理超链接
                    if (json.href && !/^xmind/.test(json.href)) {
                        obj.data.hyperlink = json.href;
                    }
                    // 处理笔记
                    if (json.notes && json.notes.plain && json.notes.plain.content) {
                        obj.data.note = json.notes.plain.content;
                    }
                    //处理子节点
                    var children = json.children && json.children.attached;
                    if (children && children.length) {
                        //多个子节点
                        obj.children = [];
                        for (var i in children) {
                            obj.children.push({});
                            processJson(children[i], obj.children[i]);
                        }
                    }
                }
                function json2km(json) {
                    // var result = {};
                    // processJson(json[0].rootTopic, result);
                    // return result;
                    // wy: mind tabs
                    var arr = [];
                    var result = {};
                    for (var i = 0; i < json.length; i++) {
                        result = {};
                        processJson(json[i].rootTopic, result);
                        result.tabName = json[i].title || "";
                        arr.push(result);
                    }
                    return arr;
                }
                ...
                function readDocument(entries) {
                    return new Promise(function(resolve, reject) {
                        ...
                        for (var i = 0, len = entries.length; i < len; i++) {
                            if (entries[i].filename.split("/").pop() == "content.json") {
                                entry = entries[i];
                                break;
                            }
                        }
