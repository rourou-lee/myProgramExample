				...
				switch (state) {
                    ...
                    case 'normal': {
                        ...
                        if (!window.lock && window.ManualEdit && window.socket) {
                            if (window.firstEdit && window.socket) {
                                window.socket.emit('editLock');
                                window.firstEdit = false;
                            }
                            window.socket.emit("execCommand", { params: ["copy"] });
                        }
			...
			if (document.activeElement == receiver.element) {
				...
				switch (state) {
                    ...
                    case 'normal': {
                        ...
                        // content:剪切同步
                        // * ====================================
                        if (!window.lock && window.ManualEdit && window.socket && window.socket) {
                            if (window.firstEdit) {
                                window.socket.emit('editLock');
                                window.firstEdit = false;
                            }
                            window.socket.emit("execCommand", { params: ["cut"] });
                        }
			...
			if (document.activeElement == receiver.element) {
				...
				switch (state) {
                    ...
                    case 'normal': {
                        var args = [];
                        args[0] = "paste";
                        args[1] = Date.now(); //Math.floor(Math.random() * 10 + 1);
                        // * ====================================
                        // add by shiyaru 
                        // date: 20180510
                        // content:黏贴同步
                        ...
                        if (!window.lock && window.ManualEdit && window.socket ) {
                            ...
                            window.socket.emit("execCommand", { params: args });
                        }
						...
						if (MimeType.whichMimeType(textData) === 'application/km') {
							...
							sNodes.forEach(function(node) {
                                ...
                                for (var i = nodes.length - 1; i >= 0; i--) {
                                    ...
                                    minder.importNode(_node, nodes[i], args[1]);
